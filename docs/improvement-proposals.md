...
**提案**: 読み上げキューの内容をアプリ再起動後も復元できるようにする（ON/OFF 設定付き）。

**実装**:
- `QueueEntry` モデル（`Label` / `Text` / `CreatedAt`）を JSON 保存
- `QueuePersistenceService.Load/Save` で `queue.json` を管理
...
#### ✅ #130 読み上げ履歴（直近 20 件）【v0.8.8 完了】

**課題**: 一度キューをクリアしたり再生完了後に再読み上げしたい場合、テキストを再入力する必要がある。

**実装**:
- 再生開始・キュー再生時に `AddHistoryEntry(text)` を呼び出し（上限 20 件、古いものから自動削除）
- `MainWindow.HistoryOperations.cs` 新規追加（`QueueEntry` モデルを再利用）
- 再生パネルに `BrdHistory` パネル追加（1件以上で自動展開）
- 「原稿へ」「＋ キュー」「クリア」ボタンで履歴を活用
- 連続同一テキストの重複エントリ防止
- セッション内のみ保持（永続化なし）
...
**提案**:
- 複数選択削除（ListBox で複数選択 → Delete キーまたは「削除」ボタン）
- ドラッグ&ドロップまたは上下移動ボタンによる並び替え
...
**延期理由**: `DictionaryEntry` スキーマ変更・既存 dictionary.json の後方互換対応・UI 変更の合計コストが v1.0.0 スコープを超える。v1.0.0 リリース後に着手を検討。

#### ⏸ #128 音声出力デバイス選択【ポスト 1.0 延期】
...
**延期理由**: エンジンごとにデバイス指定 API が大きく異なり、3 エンジン全対応のコストが高い。OS の音量ミキサーで代替可能なため v1.0.0 では見送り。
