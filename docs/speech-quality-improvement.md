...
- **読み上げ箇所ハイライト** ✅ v0.3.3 対応済み: `SpeechSynthesizerOptions.IncludeWordBoundaryMetadata = true`
  を有効にし、`MediaPlaybackItem.TimedMetadataTracks` の `SpeechCue.CueEntered` イベントから
  `SpeakProgress` を発火。SAPI モードと同等のハイライト動作を実現。
...
| VOICEVOX Core（直接利用） | 高 | 大（数百 MB） | 中（ライブラリ呼び出し） | 中〜高 |
| VOICEVOX エンジン（プロセス起動） | 高 | 大（数百 MB） | 高（別プロセス管理 + LGPL 対応） | 中 |

#### ライセンス概要（確認済み）

**OpenJTalk**

- 本体ライセンス: **Modified BSD License**（実質 BSD-3-Clause 相当）
- 同梱物に追加条件あり: MeCab・NAIST-jdic 等の辞書・音声モデルは個別の COPYING を確認する必要がある（readme に明記）。
- 閉域配布への適合度: **高**（コピーレフトなし、著作権表示と免責事項の記載で配布可能）

**VOICEVOX Core（voicevox_core）**

- 本体ライセンス: **MIT License**
- ただし同梱のモデル・辞書・依存ライブラリは別ライセンスが混在し得るため、配布物単位の棚卸しが必要。
- `voicevox_core` を直接 P/Invoke 等で利用する場合は MIT 条件が中心となる見込み。

**VOICEVOX アプリ / エンジン（voicevox リポジトリ）**

- ライセンス: **LGPL v3 + ソース公開不要の別ライセンスのデュアルライセンス**
- アプリ・エンジンをそのまま組み込む場合は LGPL の制約（動的リンク・静的リンクの扱い）を精査する必要がある。
- 本ツールに組み込む場合は `voicevox_core` 直接利用の方が扱いやすい。

#### 実務上の判断ポイント（閉域 Windows 配布向け）

1. **まず `voicevox_core` 直接利用を検討する**: `voicevox` アプリを避けることで LGPL の考慮範囲を絞れる。ただし同梱物の棚卸し（モデル・辞書・ランタイム DLL ）は必須。
2. **OpenJTalk は比較的扱いやすい**: ただし MeCab・NAIST-jdic 等の同梱物ライセンスを個別確認すること。
3. **いずれも配布物単位の最終棚卸しが必要**: 本体ライセンスだけでなく同梱するすべてのファイルを対象に確認する。
...
5. WinRT エンジン実装      ← ✅ v0.3.0 実装済み（backlog #32）
   └─ 読み上げハイライト   ← ✅ v0.3.3 実装済み（backlog #34）
