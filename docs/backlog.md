...
| # | 概要 | 優先度 | 備考 |
|---|---|---|---|
| 127 | 辞書エントリのカテゴリ/タグ | 検討中 | スキーマ変更を伴う |
| 128 | 音声出力デバイス選択 | 検討中 | WinRT/OpenJTalk のデバイス API が限定的 |
| 129 | プレビュー比較の差分ハイライト強化（変換箇所色分け＋「次の差分へ」ナビ） | 中 | 既存プレビューペインの拡張 |
| 130 | 読み上げ履歴（直近 N 件のテキスト・再読み上げ） | 低 | キューの補完機能 |
| 131 | テンプレートお気に入り固定（ピン留め） | 低 | テンプレート一覧の並び順拡張 |
| 132 | キュー操作小改善（複数選択削除・上下移動・重複除去） | 低 | キュー UI の細部改善 |
