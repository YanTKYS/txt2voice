...
| 36 | BuildAppSettingsTests の追加（backlog #36 残） | `BuildAppSettings(isExit)` は UI コントロール（`TxtInput`, `SldRate` 等）への依存が大きく単体テストが困難。ViewModel 層への分離か、WPF テストフレームワーク経由でのテストを検討。`SpeechEngineFactoryTests` は v0.3.3 で追加済み。 |
...
| 37 | WinRT 保存処理の長文メモリ効率改善（#37 残） | v0.3.3 で `using` 解放と事前確保は対応済み。長文対応として一時 WAV ファイル経由のストリーミングエンコード（`MemoryStream` 廃止）を別途検討。 |
