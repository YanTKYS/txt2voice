...
| 52 | OpenJTalk セットアップのオフライン完結強化（残: 同梱ファイル優先・URL 複数候補化） | `--verify-only` と UI 診断チェックリストは v0.4.5 で完了。未着手: 同梱ファイル優先・取得元 URL 複数候補化。詳細 #52。 |
...
| 63 | ログ匿名化ハードニング | テキスト本文をログに記録しないよう API を見直し。メタ情報のみ（エンジン・文字数・保存パス）をログ対象とする方針の明文化。詳細 #63。 |
