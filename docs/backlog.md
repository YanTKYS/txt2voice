...
| 120 | テンプレート利用履歴（最近使った順ソート） | 高 | LastUsedAt/UsageCount をモデルに追加 |
| 121 | テンプレート予約変数（{today}/{now} 等） | 高 | PlaceholderDialog の自動初期値 |
| 122 | 保存プリセット | 高 | 形式・命名・SSML・速度をまとめて保存 |
| 123 | プレビュー比較ペイン（辞書適用前後） | 中 | 行同期スクロール |
| 124 | 段落/行番号表示トグル | 中 | 設定から ON/OFF |
| 125 | ショートカット拡張（Alt 系） | 中 | Alt+↑↓/Alt+Shift+↑↓/Alt+←→ |
