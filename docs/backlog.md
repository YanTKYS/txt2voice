...
| # | 概要 | 詳細 |
|---|---|---|
| 36 | BuildAppSettingsTests の追加 | `BuildAppSettings(isExit)` は UI コントロール（`TxtInput`, `SldRate` 等）への依存が大きく単体テストが困難。ViewModel 層への分離か、WPF テストフレームワーク経由でのテストを検討。`SpeechEngineFactoryTests` は v0.3.3 で追加済み。 |
