...
| 31 | ISpeechEngine 抽象化（音声エンジン差し替え可能化） | ISpeechEngine インターフェースを新設し SystemSpeechEngine に切り出す。#32/#33 の前提ステップ。設定で「音声エンジン種別」を選択可能にする。 |
...
| 32 | WinRT 音声エンジン実装（第一候補） | #31 完了後。Windows.Media.SpeechSynthesis を使う WinRtSpeechEngine を実装。端末の OneCore 音声を利用可。 |
