...
なし — v0.8.10 時点でバックログは完了。

---

## ポスト 1.0 延期

v1.0.0 スコープ外として正式に延期。技術的制約またはスコープ超過が理由。

| # | 概要 | 理由 |
|---|---|---|
| 127 | 辞書エントリのカテゴリ/タグ | スキーマ変更・後方互換対応が大きく v1.0 スコープ超過 |
| 128 | 音声出力デバイス選択 | WinRT/OpenJTalk のデバイス API が限定的で実装困難 |
...
| 120 | テンプレート利用履歴（最近使った順ソート + LastUsedAt/UsageCount） | v0.7.9 |
| 122 | 保存プリセット（単体/一括・命名テンプレート・SSML 強度） | v0.8.0 |
| 125 | Alt ショートカット（段落/セクション/プレビューナビ） | v0.8.0 |
| 119 | 読み上げ進捗インジケータ＋残り時間表示 | v0.8.1 |
| 124 | 段落/行番号表示トグル | v0.8.2 |
| 123 | プレビュー比較ペイン（辞書適用前後） | v0.8.3 |
| TD-1 | SpeakAsync/SpeakSsmlAsync → Task 戻り値（再生 API awaitable 化） | v0.8.4 |
| 126 | 読み上げキュー（簡易版） | v0.8.5 |
| キュー永続化 | 読み上げキューのアプリ再起動後復元（ON/OFF 設定付き） | v0.8.6 |
| 129 | プレビュー比較の変換箇所ハイライト（色分け＋◀▶ナビ） | v0.8.7 |
| 130 | 読み上げ履歴（直近 20 件・原稿復元・キュー追加） | v0.8.8 |
| 131 | テンプレートお気に入り固定（ピン留め） | v0.8.9 |
| 132 | キュー操作小改善（複数選択削除・上下移動・重複除去） | v0.8.10 |
