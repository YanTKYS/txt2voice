...
| # | 概要 | 優先度 | 備考 |
|---|---|---|---|
| 119 | 読み上げ進捗インジケータ＋残り時間表示 | 高 | SAPI/WinRT/OpenJTalk 各エンジン対応 |
| TD-1 | SpeakAsync/SpeakSsmlAsync → Task 戻り値（再生 API awaitable 化） | 高 | #126 読み上げキューの前提条件・各エンジン変更を伴う |
| 123 | プレビュー比較ペイン（辞書適用前後） | 中 | 行同期スクロール |
| 124 | 段落/行番号表示トグル | 中 | 設定から ON/OFF |
| 126 | 読み上げキュー（簡易版）| 検討中 | #119 完了後に再評価 |
